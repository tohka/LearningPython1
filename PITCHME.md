...
文字は、たとえば `a` であれば `97` 、 `あ` であれば `12354` といったように文字に数値を割り当てて管理している。この対応表のことを文字コードと呼び、様々な種類が存在する。

一方で、そのコードを符号化する方法（エンコーディング）は更に複数の種類が存在する。

| 文字 | Unicode | UTF-8 | UTF-16 BE |
|:----:|--------:|------:|----------:|
|  a   | <tt>1100001</tt> | <tt>01100001</tt> | 00000000 01100001</tt> |
|  あ  | <tt>110000 01000010</tt> | <tt>1110<u>0011</u> 10<u>000001</u> 10<u>000010</u></tt> | <tt>00110000 01000010</tt> |

Python 3.x の文字列は、文字コードとしての Unicode 列として管理され、入出力の際に適切にエンコーディング変換される。

スクリプト内で文字列から特定のエンコーディングにおけるバイト列に変換するには `"string".encode(encoding)` を使用する。
...
シーケンス型とは、順番をもった複数の要素を扱った型のことで、特定の型ではなく、同種の特徴をもった文字列、バイト列、後述のリストやタプル等を指す。

以下の関数等はシーケンス型で共通して使用できる。
